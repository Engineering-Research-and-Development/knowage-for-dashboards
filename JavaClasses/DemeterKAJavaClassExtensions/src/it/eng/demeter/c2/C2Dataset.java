...
public class C2Dataset extends DemeterAbstractJavaClassDataSet /*implements it.eng.spagobi.tools.dataset.bo.IJavaClassDataSet*/ {
	...
	public String getValues(Map profile, Map parameters) {
		...
		try {
			...
			logger.warn("No  \"DATASET_CONTENT\" finded in parameters! \"PARCEL\" value will be used!\n" + e.getMessage(), e.getCause());
			e.printStackTrace();
		}
	...
	protected String aimTranslator(StringBuilder aim) throws Exception, JSONException {
		...
		switch (dsContent.toUpperCase()) {
		case "PARCEL":
			ds = aimReaderForC2_Parcel(aim);
			break;
		case "ZONE":
			ds = aimReaderForC2_Zone(aim);
			break;
		case "WEATHER":
			ds = aimReaderForC2_Weather(aim);
			break;
		}
	...
	public static String aimReaderForC2_Parcel(StringBuilder aim) throws Exception, JSONException {
		/*Requesting AIM*/
	...
	public static String aimReaderForC2_Zone(StringBuilder aim) throws Exception, JSONException {
		/*Requesting AIM*/
		/*StringBuilder aim = new StringBuilder();
	      URL url = new URL(urlToRead);
	      HttpURLConnection conn = (HttpURLConnection) url.openConnection();
				...
				case "Farm":
					...
					for (int j=0; j<parcels.length(); j++) {
						...
						for (int x=0; x<containsZones.length(); x++) {
							...
							/*Acquisizione quantit� per classe e totale*/
	...
	public static String aimReaderForC2_Weather(StringBuilder aim) throws Exception, JSONException {
		/*Requesting AIM*/
		/*StringBuilder aim = new StringBuilder();
	      URL url = new URL(urlToRead);
	      HttpURLConnection conn = (HttpURLConnection) url.openConnection();
